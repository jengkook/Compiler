...
use crate::ast::{AssignExpr, Ast, BinaryExpr, BinOpKind, BlockExpr, BoolExpr, CallExpr, Expr, FunctionDeclaration, IfExpr, ItemId, LetStmt, NumberExpr, ParenthesizedExpr, Stmt, UnaryExpr, UnOpKind, VarExpr, WhileStmt};
...
impl<'a> ASTVisitor for ASTEvaluator<'a> {
    ...
    fn visit_assignment_expression(&mut self, ast: &mut Ast, assign_expr: &AssignExpr, _expr: &Expr) {
        self.visit_expression(ast, assign_expr.expression);
        self.frames.update(assign_expr.variable_idx, self.last_value.unwrap());
    }

    fn visit_variable_expression(&mut self, _ast: &mut Ast, var_expr: &VarExpr, _expr: &Expr) {
